...
class RegisterManger(models.Manager):

    def new_or_get(self, user, cart_obj):
        created =False
        qs = self.get_queryset().filter(user=user, cart=cart_obj, status="ایجاد شده")
    ...
    def get_user_registers(self,user):
        active_classes = []
        qs = self.get_queryset().filter(user=user, status="پرداخت شده")
...
class Register(models.Model):
        ...
        status = models.CharField(max_length=120, default='ایجاد شده', choices=Status.choices)
        total = models.DecimalField(default=0.00, max_digits=100, decimal_places=2)
        register_date = models.DateField(blank=True, null=True)
        expiration_date = models.DateField(blank=True, null=True)
        is_expired = models.BooleanField(default=False)

        objects = RegisterManger()
        ...
        def mark_paid(self):
            if self.check_done():
                self.status = "پرداخت شده"
                self.save()
            return self.status


def add_dates(sender, instance, *args, **kwrags):  
...
def pre_save_create_register_id(sender, instance, *args, **kwrags):
